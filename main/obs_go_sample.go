...
func setBucketNotification() {
	...
	topicConfigurations[0].Topic = "urn:smn:region3:35667523534:topic1"
	topicConfigurations[0].Events = []string{"s3:ObjectCreated:*"}
...
func setObjectMetadata() {
	...
	input.Metadata = map[string]string{"test": "value"}
	output, err := getObsClient().SetObjectMetadata(input)
	if err == nil {
		fmt.Printf("StatusCode:%d, RequestId:%s\n", output.StatusCode, output.RequestId)
	} else {
		if obsError, ok := err.(obs.ObsError); ok {
